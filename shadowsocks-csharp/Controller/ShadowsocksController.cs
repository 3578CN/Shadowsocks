...
namespace Shadowsocks.Controller
{
    public class ShadowsocksController
    {
        ...
        private static readonly string[] DefaultConnectivityProbeUrls = new[]
        {
        ...
        {
            public ConnectivityChangedEventArgs(ConnectivityStatus status)
            {
                Status = status;
            }

            public ConnectivityStatus Status { get; }
        }
        ...
        public event EventHandler<ConnectivityChangedEventArgs> ConnectivityStatusChanged;
        ...
        public ShadowsocksController()
        {
            ...
            _connectivityMonitor = new ConnectivityMonitor(TimeSpan.FromSeconds(20), ProbeConnectivityAsync);
        ...
        {
            UpdateConnectivityStatus(isConnected ? ConnectivityStatus.Online : ConnectivityStatus.Offline);
        }
        ...
        private void UpdateConnectivityStatus(ConnectivityStatus status)
        {
            ...
            ConnectivityStatusChanged?.Invoke(this, new ConnectivityChangedEventArgs(status));
        }

        public ConnectivityStatus GetConnectivityStatus() => _connectivityStatus;

        private void EnsureConnectivityMonitor(Configuration config)
        {
            ...
            if (config.enabled)
            {
                UpdateConnectivityStatus(ConnectivityStatus.Unknown);
                _connectivityMonitor.Start();
            }
            else
            {
                _connectivityMonitor.Stop();
                UpdateConnectivityStatus(ConnectivityStatus.Disabled);
            }
        }

        private async Task<bool> ProbeConnectivityAsync(CancellationToken cancellationToken)
        {
            ...
            int port = privoxyRunner.RunningPort;
            if (port <= 0)
            {
                return false;
            }

            var proxyUri = new Uri($"http://127.0.0.1:{port}");
            ...
            using (var handler = new HttpClientHandler())
            {
                ...
                using (var client = new HttpClient(handler, true))
                {
                    ...
                    foreach (var url in DefaultConnectivityProbeUrls)
                    {
                        ...
                        try
                        {
                            using (var request = new HttpRequestMessage(HttpMethod.Get, url))
                            {
                                ...
                                request.Headers.UserAgent.ParseAdd("Shadowsocks-Connectivity/1.0");
                        ...
                        {
                            logger.Debug(ex, $"Connectivity probe to {url} failed");
                        }
                    }
                }
            }
        ...
        protected void Reload()
        {
            ...
            UpdateSystemProxy();
            EnsureConnectivityMonitor(_config);
        }
