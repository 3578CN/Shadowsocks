...
namespace Shadowsocks.View
{
    public class MenuViewController
    {
        ...
        private Icon icon, icon_in, icon_out, icon_both, icon_warn, previousIcon;

        private bool _isStartupCheck;
        private string _urlToOpen;
        private ShadowsocksController.ConnectivityStatus _connectivityStatus = ShadowsocksController.ConnectivityStatus.Unknown;
        ...
        private readonly Color colorMaskWarning = Color.FromArgb(255, 255, 204, 0);
        private readonly Color colorMaskDisabled = Color.FromArgb(255, 128, 128, 128);
        ...
        public MenuViewController(ShadowsocksController controller)
        {
            ...
            controller.ConnectivityStatusChanged += controller_ConnectivityStatusChanged;
            _connectivityStatus = controller.GetConnectivityStatus();
        ...
        private void UpdateTrayIconAndNotifyText()
        {
            ...
            Bitmap warnBitmap = ViewUtils.ChangeBitmapColor(Resources.ss32Fill, colorMaskWarning);
            warnBitmap = ViewUtils.AddBitmapOverlay(warnBitmap, Resources.ss32Outline);
            icon_warn = Icon.FromHandle(ViewUtils.ResizeBitmap(warnBitmap, iconSize.Width, iconSize.Height).GetHicon());

            previousIcon = _connectivityStatus == ShadowsocksController.ConnectivityStatus.Offline ? icon_warn : icon;
            ...
            // If system proxy is disabled, explicitly show disabled status instead of checking connectivity
            string connectivityText;

            switch (_connectivityStatus)
            {
                case ShadowsocksController.ConnectivityStatus.Online:
                    connectivityText = I18N.GetString("ConnectivityStatusOnline");
                    break;
                case ShadowsocksController.ConnectivityStatus.Offline:
                    connectivityText = I18N.GetString("ConnectivityStatusOffline");
                    break;
                case ShadowsocksController.ConnectivityStatus.Disabled:
                    connectivityText = I18N.GetString("ConnectivityStatusDisabled");
                    break;
                default:
                    connectivityText = I18N.GetString("ConnectivityStatusChecking");
                    break;
            }

            if (!enabled && _connectivityStatus != ShadowsocksController.ConnectivityStatus.Disabled)
            {
                connectivityText = I18N.GetString("ConnectivityStatusDisabled");
            }
        ...
        private void controller_TrafficChanged(object sender, EventArgs e)
        {
            ...
            if (_connectivityStatus == ShadowsocksController.ConnectivityStatus.Offline)
            {
                newIcon = icon_warn ?? icon;
            }
        ...
        private void controller_ConnectivityStatusChanged(object sender, ShadowsocksController.ConnectivityChangedEventArgs e)
        {
            _connectivityStatus = e.Status;
            UpdateTrayIconAndNotifyText();
        }
